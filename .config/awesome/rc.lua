...
--beautiful.hotkeys_font = "Inconsolata 8"
--beautiful.hotkeys_description_font = "Inconsolata 8"
...
run_once ({"dunst", "tilda -g /home/evas/.config/tilda/config_0 -h","/home/evas/bin/setxkbmap-greek","nextcloud","/home/evas/bin/greenclip daemon","xkbset bo 30","/usr/bin/xcalib -d :0 /home/evas/bin/dell_u2515h.icc"})
...
local show_clipboard= "rofi -modi 'clipboard:/home/evas/bin/greenclip print' -show clipboard  -run-command '{cmd}' -theme android_notification -lines 20 -opacity 60 -font \"Inconsolata 12\""
...
beautiful.init(string.format("%s/.config/awesome/themes/winterland/theme.lua", os.getenv("HOME")))
-- }}}
...
globalkeys = my_table.join(
    ...
    awful.key({ modkey,           }, "j",
       function()
	  local t = awful.tag.selected(1).name
	  if t > "1" then awful.tag.viewprev() end
       end,
    ...
    -- Suspend and stuff
    awful.key({ modkey }, "F2", function () awful.spawn(suspend_mon) end,
       {description = "Put monitor to sleep", group = "hotkeys"}),
    awful.key({ modkey }, "F1", function () awful.spawn(suspend) end,
       {description = "Suspend to ram", group = "hotkeys"}),
    awful.key({ modkey }, "i", function () awful.spawn(pushover_i) end,
       {description = "Send clipboard to iphone", group = "hotkeys"}),
    ...
    awful.key({ modkey }, "p",
        function ()
            os.execute("mpc -h 192.168.0.1 toggle")
    ...
    awful.key({ modkey }, "bracketleft",
        function ()
            os.execute("mpc -h 192.168.0.1 prev")
            beautiful.mpd.update()
        end,
        {description = "mpc prev", group = "widgets"}),
    awful.key({ modkey,  }, "bracketright",
        function ()
            os.execute("mpc -h 192.168.0.1 next")
            beautiful.mpd.update()
        end,
    ...
    awful.key({ modkey }, "w", function () awful.spawn(rofi_win_list) end,
    ...
    awful.key({ modkey }, "e", function () awful.spawn(show_clipboard) end,
...
-- raise window if requested from rofi window list
